    ...
    return (
        <div class="relative">
            ...
            <div class="bg-[#1E225AB2] absolute top-2/4 left-2/4 max-w-[919px] translate-x-[-50%] translate-y-[-50%] py-6 px-4 lg:p-8 rounded-[10px] w-[90%] lg:w-full">
                <span class="text-white text-[20px] lg:text-2xl mb-4 lg:mb-12 block font-bold">{title}</span>
                    ...
                    {list?.map((item) => (
                        <li class="flex gap-6">          {item?.image &&
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm lg:text-base text-white font-bold">{item?.title}</span>
                                <ul class="list-disc pl-4">
                                    {item?.content?.map((text) => (
                                        <li class="marker:text-white">
                                            <span class="text-xs lg:text-sm text-white">{text}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
